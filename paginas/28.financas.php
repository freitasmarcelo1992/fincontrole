...
if (!$conexao) {
    ...
    echo '</head><body><main class="box"><h1>Conexão instável</h1><p>Não foi possível carregar seu dashboard agora. Aguarde alguns segundos e tente novamente.</p><a href="28.financas.php">Tentar novamente</a></main></body></html>';
    exit();
}
...
if ($dataIni === "" && $dataFim === "" && $filtroMes === "" && $filtroAno === "") {
    $filtroMes = date("m");
    $filtroAno = date("Y");
} elseif ($filtroMes !== "" && $filtroAno === "") {
    $filtroAno = date("Y");
}
...
$dashboardCacheChave = fincontrol_cache_dashboard_chave($id_usuario, [
    ...
    "cache" => "financas-dashboard-20260903",
]);
...
<style>
  ...
  body.financas-view .app-dashboard-filter-button{display:flex!important;align-items:center;justify-content:center;gap:8px;border:1px solid rgba(34,199,242,.38);background:rgba(18,43,67,.96);color:#dff7ff;box-shadow:none}
  body.financas-view .filters-panel{border:1px solid rgba(116,154,190,.18);background:rgba(18,43,67,.96);box-shadow:none}
  body.financas-view .filters-panel label{color:#b8c8d8}
  body.financas-view .filters-panel input,body.financas-view .filters-panel select{border-color:rgba(116,154,190,.2);background:rgba(6,24,38,.72);color:#fff}
  body.financas-view .filters-panel button{background:#14b8e8;color:#fff}
  body.financas-view .filters-panel a{background:rgba(255,255,255,.08);color:#dff7ff}
  @media(max-width:760px){body.financas-view .app-dashboard-filter-button{min-height:42px;margin:0 0 8px;border-radius:12px}body.financas-view .filters-panel{display:none!important;grid-template-columns:1fr 1fr;gap:8px;margin:0 0 8px;padding:10px;border-radius:12px}body.financas-view .filters-panel div{min-width:0}body.financas-view .filters-panel input,body.financas-view .filters-panel select{width:100%;min-height:36px;font-size:12px}body.financas-view .filters-panel button,body.financas-view .filters-panel a{min-height:36px;font-size:12px}body.financas-view .app-dashboard-filter-toggle:checked + .app-dashboard-filter-button + .filters-panel{display:grid!important}}
</style>
<?php echo function_exists("fincontrol_version_styles") ? fincontrol_version_styles() : ""; ?>
</head>

<body class="dashboard-app financas-view">
...
<aside class="sidebar" id="menu-completo">
    <div class="brand">
        ...
        <a href="28.financas.php" class="active"><i class="fa fa-wallet"></i> Finanças</a>
...
<main class="main">
    ...
    <form class="filters-panel" method="GET" action="28.financas.php">
        <div>
        ...
        <a href="28.financas.php">
            <i class="fa-solid fa-rotate-left"></i>
            Limpar
        </a>
    </form>

    <section class="app-finance-overview" aria-label="Resumo financeiro">
        <div class="app-balance-card">
            <div>
                <span>Saldo</span>
                <strong class="fc-sensitive-value"><?php echo moedaDashboard($saldo); ?></strong>
                <small>Saldo no filtro atual</small>
            </div>
            <button type="button" class="app-visibility-toggle" data-fc-toggle-values aria-label="Ocultar ou mostrar valores">
        ...
        <div class="app-summary-card">
            <div class="app-summary-item">
                <span>Receita</span>
                <strong class="fc-sensitive-value"><?php echo moedaDashboard($totalReceitas); ?></strong>
                <i class="fa-solid fa-arrow-trend-up"></i>
            </div>
            <div class="app-summary-item">
                <span>Despesa</span>
                <strong class="fc-sensitive-value"><?php echo moedaDashboard($totalSaidas); ?></strong>
                <i class="fa-solid fa-arrow-trend-down"></i>
            </div>
            <div class="app-summary-item gasto-diario">
                <span>Saldo diário</span>
                <strong class="fc-sensitive-value"><?php echo moedaDashboard($saldo / max(1, intval(date("t")))); ?></strong>
                <i class="fa-solid fa-calendar-day"></i>
            </div>
        </div>
    </section>

    <nav class="app-entry-actions" aria-label="Ações financeiras rápidas">
        <a class="receita" href="05.receitas.php">
            <i class="fa-solid fa-plus"></i>
            <span>Receita</span>
        </a>
        <a class="despesa" href="04.despesas.php">
            <i class="fa-solid fa-plus"></i>
            <span>Despesa</span>
        </a>
